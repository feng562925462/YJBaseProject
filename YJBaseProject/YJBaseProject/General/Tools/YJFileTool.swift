...
class YJFileTool: NSObject {
    ...
    // MARK:- 获取单个文件的大小
    /// 获取单个文件的大小
    class func yj_getFileSize(atPath filePath : String) -> CGFloat {
        guard yj_isExist(atPath: filePath) else {
            return 0
        }
    ...
    class func yj_folderSizeAtPath(folderPath:String) -> CGFloat
    {
    ...
    class func yj_folderDelete(folderPath:String) {
        ...
        for path in childFilePaths {
            let bool = self.yj_delete(atPath: folderPath + path)
            printLog("删除结果：\(bool)")
